    ...
    internal class Program
    {
        ...
        static void Functions(short menu, int[,] myArray, int[,] backArray)  // Menu pro uživatele
        {
            while (true)
            {
                try
                {
                    ...
                    Console.WriteLine($"Zvol funkci, kterou chceš. Napiš:\n1 pro sečtení vygenerované (původní) matice\n2 pro vynásobení součtu prvků kvocientem\n3 pro vynásobení součtem prvků jiné matice\n4 pro přehození čísel na hlavní diagonále\n5 pro překlopení čísel přes hlavní diagonálu");
        ...
        static int[,] CreateArray() // Menu pro vytvoření matice matice
        {
            try
            {
                ...
                Console.WriteLine("Nyní zadej délku strany čtvercové matice:");
        ...
        static int[,] randArray(int size) // Vytvoření náhodné matice
        {
        ...
        static int[,] orderedArray(int size) // Vytvoření posloupné matice
        {
            try
            {
                int num = 1;
        ...
        {
            int[,] tempArray = new int[myArray.GetLength(0), myArray.GetLength(0)];
            for (int i = 0; i < myArray.GetLength(0); i++)
            {
                for (int j = 0; j < myArray.GetLength(1); j++)
                {
